...
const UploadProducts = () => {
  ...
  const handleSubmit = async () => {
    if (
      ...
      setError("Title, Price, Category, and Images are required");
      return;
    }

    try {
      setLoading1(true);
      const response = await addProduct(productData, user.token);
      toast.success(response.message);
      setLoading1(false);
      window.location.reload();
    } catch (error) {
      setError2(error.response?.data?.error?.message || "An error occurred");
    }
  };
...
// Inside the return statement, update the mapping of images to include a remove button
    ...
    <div className="flex h-[100vh] ">
      ...
      <div className="flex-[8] pt-[50px] pl-[30px] pr-[80px] Lato">
        <div className="flex flex-col gap-[20px]">
          ...
          >
            {loading1 ? <CircleLoader className="text-white" /> : "Add Product"}
          </button>
        </div>
      </div>
    </div>
